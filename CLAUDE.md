...
- **`RpcMethodInfo`** / **`MethodType`** / **`ServiceIntrospector`** — reflective introspection of a service interface. Pulls method type (UNARY/STREAM), params schema, result schema, auth requirements, and the protocol's **wire name**: `@ProtocolName("…")` when declared, else the interface's simple name. Read from the interface's *own* annotations (`getDeclaredAnnotation`), mirroring the reference's `vars(protocol)` lookup — a subinterface that silently inherited its parent's routing key would impersonate it. A declared name is validated against `ProtocolNames.isValid` and refused if it claims the reserved `vgi_rpc.` prefix; `RpcServer` resolves it in its constructor, so an unroutable declaration fails at worker construction rather than on every request. Declare it whenever the name is a cross-implementation contract: the VGI protocol is `vgi.v2`, and no Java simple name can carry the dot.
...
- **`transport/`** — `RpcTransport` interface, `StdioTransport`, `SubprocessTransport`, `UnixSocketTransport`, `TcpSocketTransport` (raw Arrow-IPC framing over a bare TCP socket — the network analog of `UnixSocketTransport`; no auth/TLS, loopback-default, trusted networks only).
  ...
  **A full AF_UNIX accept queue is a busy peer, not a dead one.** Under a connection burst Linux fails a non-blocking connect with `EAGAIN`, which the JDK surfaces as a bare `SocketException` ("Resource temporarily unavailable") — and a *blocking* `SocketChannel.connect` behaves differently by thread: it waits unboundedly on a platform thread and fails at once on a virtual one (the JDK makes a virtual thread's socket non-blocking). Measured on Linux, JDK 21 and 25. So client code connects with `UnixSocketTransport.connect(path, timeout)`, which connects non-blocking and retries `EAGAIN` with a capped backoff until the timeout, and `LauncherClient.probe` counts `EAGAIN` as alive and re-probes a refusal at 50/100/200 ms (macOS reports a full queue as `ECONNREFUSED`) — reading a busy worker as dead made the launcher unlink its socket and spawn a duplicate. `UnixSocketTransport.isAcceptQueueFull` tells `EAGAIN` apart by exception *type* plus "the path is still a socket inode", never by the localised message. Linux-only tests: `UnixSocketConnectTest`, `LauncherBusyWorkerTest`.
- **`http/`** — Jetty-based HTTP transport. `HttpServer`, `HttpPreHandler`, `HttpStreamHandler` (stateless streaming: state travels in a signed `StateToken` in custom metadata), `StateSerializer`, `StateToken`, `Authenticator`, `AuthException`, `TokenExpiredException`.
  ...
  **Token introspection is `vgi_rpc.Identity.v1`, nothing else.** The pre-0.46 `POST {prefix}/__introspect_token__` JSON route, its `TokenIntrospection`/`TokenResolver`/`http.TokenIdentity` classes, its `Config.tokenIntrospection(...)` / `introspectTtlSeconds` / `introspectRateLimitPerSecond` options and its `VGI-Token-Introspection` capability header are **retired** (reference `IDENTITY_V1_SPEC.md` §8): two surfaces meant two sets of guards to keep identical, and the second had already drifted. The path now answers as an unimplemented reserved method. Configure introspection with `RpcServer.setIdentity(IdentityImpl.builder()…)`; a client learns whether a worker introspects from reflection. **Introspection is not rate limited** (spec §4, "No rate limiter"): the allowlist is the control, and a per-caller budget was one budget for every user behind the asking proxy, drainable by unauthenticated junk credentials. A throttle anyone adds anyway must answer `identity_unavailable` (transient), never `introspection_refused` (definitive, cacheable). The cross-port audit forbids a `RateLimiter` in identity code; conformance pins it with `TestIntrospectionIsNotThrottled`.
